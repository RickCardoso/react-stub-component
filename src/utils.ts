...
export const mockComponentTestId = (uniqueName: string): string => `mock-component-${uniqueName}`;
export const reactNodeTestId = (uniqueName: string, propName: string): string =>
  `react-node-${uniqueName}-${kebabCase(propName)}`;

export const getEventName = (uniqueName: string, eventKey: string): string =>
  compact([uniqueName, kebabCase(eventKey)]).join('-');
